...
- 키별 설정: 사용 여부, 모니터링 키, 알림음, 쿨다운 시간(초, 최대 999), 알림 시간, 재누름 방지
- `＋ 키 추가` 버튼으로 행 추가 (행이 늘면 창이 커지고, 일정 크기 이상이면 스크롤)
- 타이머 종료 전 알림 시점 설정
- 여러 개의 알림음
- 키 라벨 전체를 진행바로 사용하는 카운트다운 표시 (남은 시간은 초 단위, 최대 999)
- 창 항상 위 고정
- 한국어/영어 전환
...
3. 행에서 알림음, 쿨다운 시간(초), 알림 시간(초)을 설정합니다. 행 맨 앞 체크박스로 사용 여부를 켜고 끌 수 있습니다.
